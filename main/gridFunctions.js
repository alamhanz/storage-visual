    ...
    fetch('data.csv')
        ...
        .then(csvText => {
            ...
            parsedData.data.forEach((row, index) => {
                ...
                // Add an event listener to highlight the corresponding box and row on click
                tr.addEventListener('click', () => {
                    highlightBox(row['Box ID']);
                    highlightRow(tr);
                });
            ...
            // Add event listeners to table headers to reset the boxes and row highlights on click
...
function highlightRow(selectedRow) {
    ...
    rows.forEach(row => {
        if (row === selectedRow) {
            row.style.backgroundColor = '#dae9d3';
            row.style.display = ''; // Show the selected row
        } else {
...
function highlightBox(boxId) {
    Object.keys(window.boxes).forEach(key => {
        ...
        if (box.userData.boxId === boxId) {
            box.material.opacity = 1.0;  // Highlight the box
        } else {
            box.material.opacity = 0.0;  // Hide all other boxes
        }
    });
}
...
function updateBoxesAndChart(data, colorsMap, gridSections, gridWidth, gridHeight, gridDepth, boxSize, spacing) {
    ...
    data.forEach((row, index) => {
        ...
        if (box) {
            ...
            box.userData.boxId = row['Box ID'];  // Store the Box ID in userData
        }
    });
